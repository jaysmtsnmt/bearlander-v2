...
class Agent():
    ...
    def login(self):
        ...
        try: 
            ...
            try: #check for incorrect login detals
                ...
                raise E.IncorrectDetails(self.username, self.password)
    ...
    def scrapeTimetable(self):
        if not self.isLoggedIn():
            ...
            with open("cache/response.txt", "w") as file: file.write(response.text) #dumping response
    
            if "var data = " in response.text:
                with open("cache/response.txt", "r") as file:
            ...
            with open(f"Bearlander/cache/data/{self.user.id}/{date.strftime(r"%m-%d-%Y")}.pickle", "wb") as file: pass #pickle.dump(, file) 

if __name__ == "__main__":
    from dataHandler import User

    try: #attempts to create user.
        user = User("jaydsoh@gmail.com", "****", "Jayden Soh Guan Shun")

        # use case
        try:
            agent = Agent(user)
            agent.login()
            agent.scrapeTimetable()


        except E.AgentError:
            print(f"Agent Error")

    except E.IncorrectDetails:
        pass
